...
SCANNET_LABELS = ['unannotated', 'wall', 'floor', 'chair', 'table', 'desk', 'bed', 'bookshelf', 'sofa', 'sink',
...
if __name__ == "__main__":
    ...
    for scene_id in tqdm(scene_list):
        ...
        # load frames
        frame_list = list(
            map(lambda x: x.split(".")[0], sorted(os.listdir(SCANNET_FRAME_ROOT.format(scene_id, "color")))))
        ...
        for i, frame_id in enumerate(frame_list):
            scene_images[i] = load_image(SCANNET_FRAME_PATH.format(scene_id, "color", "{}.jpg".format(frame_id)),
                                         [328, 256])
            scene_depths[i] = load_depth(SCANNET_FRAME_PATH.format(scene_id, "depth", "{}.png".format(frame_id)),
                                         [41, 32])
        ...
        for i, projection in enumerate(projections):
            ...
            if args.gt:
                feat = to_tensor(load_image(ENET_GT_PATH.format(scene_id, "labelv2", "{}.png".format(frame_id)),
                                            [41, 32])).unsqueeze(0)
            else:
                image = load_image(SCANNET_FRAME_PATH.format(scene_id, "color", "{}.jpg".format(frame_id)), [328, 256])
                feat = enet(to_tensor(image).unsqueeze(0)).max(1)[1].unsqueeze(1)

            proj_label = PROJECTOR.project(feat, projection_3d, projection_2d, scene.shape[0]).transpose(1,
                                                                                                         0)  # num_points, 1
